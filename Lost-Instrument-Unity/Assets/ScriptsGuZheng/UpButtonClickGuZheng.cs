...
public class UpButtonClickGuZheng : MonoBehaviour
{
    public bool isUp;
    public Sprite[] imgs;
    ...
    public void ButtonOnClick()
    {
        ...
        if (!isUp)
        {
            aim.GetComponent<GuZhengTotalMove>().moveUpStart();
            //text.text = "Down";
            this.GetComponent<Image>().sprite = imgs[1];
        }
        else
        {
            aim.GetComponent<GuZhengTotalMove>().moveDownStart();
            //text.text = "Up";
            this.GetComponent<Image>().sprite = imgs[0];
        }
        isUp = !isUp;

        GameObject.Find("Main Camera").GetComponent<ScrollLeftAndRightForCameraMoveGuZheng>().changeCanScroll();
    }
