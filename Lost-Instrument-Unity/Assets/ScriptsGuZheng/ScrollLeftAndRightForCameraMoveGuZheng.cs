...
public class ScrollLeftAndRightForCameraMoveGuZheng : MonoBehaviour
{
    ...
    public void ScrollLeft(bool toLeft)
    {
        if ((toLeft && tmpIndex + 1 < aimPositions.Length) || (!toLeft && tmpIndex - 1 >= 0))
        {
            ...
            //angle = 0f;
            aimObject.GetComponent<ScrollLeftAndRightForRotateGuZheng>().startRotateLeft(toLeft);
        ...
        if (!toLeft)
        {
            GameObject.Find("ContentGroup").GetComponent<ContentScriptGuZheng>().rightSwipeCallBack();
        }
        else
        {
            GameObject.Find("ContentGroup").GetComponent<ContentScriptGuZheng>().leftSwipeCallBack();
        }
